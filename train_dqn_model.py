# Create a DQN algorithm object using a config object ..
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in ('proportion', 'sum'):
        ...
        for i in range(NUM_TRAINING_ITERATIONS):
            ...
            if (i + 1) % 10 == 0:
                path_to_checkpoint = my_dqn.save()
                print("*********************************")
                print(
        ...
        final_checkpoint = my_dqn.save('./models/')
        renamed_checkpoint = f'models/{prefix}_dqn_{NUM_TRAINING_ITERATIONS}_model_{reward_function}_{final_checkpoint.split("/")[-1]}_{datetime.now().strftime("%Y%m%d%H%M%S%f")}'

        os.rename(final_checkpoint, renamed_checkpoint)

        my_dqn.stop()
        print(f'Final DQN checkpoint (reward function: {reward_function}): {renamed_checkpoint}')